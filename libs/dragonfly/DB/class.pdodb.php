...
class PDODb
{
    ...
    public static function getInstance()
    {
        ...
        if (is_null(self::$me)) {
            ...
            self::$me = new PDODb(
                array(
                    'type' => PDODb::getDatabseType($port),
                    'host' => $config['db_host'],
    ...
    private function buildInsert($tableName, $insertData, $operation)
    {
        ...
        $stmt = $this->buildQuery(array(), $insertData);
        $status = $stmt->execute();
    ...
    private function buildInsertQuery($tableData)
    {
        ...
        if ($isInsert) {
            if (isset($dataColumns[0]))
                $this->query .= ' (`' . implode($dataColumns, array('`, `')) . '`) ';
            $this->query .= ' VALUES (';
        } else {
            $this->query .= " SET ";
        }
     ...
     * @return null | PDOStatement Returns the $stmt object.
     */
    ...
    public function get($tableName, $numRows = null, $columns = '*')
    {
        ...
        if (in_array('SQL_CALC_FOUND_ROWS', $this->queryOptions)) {
            if ($dbType == 'mysql') {
                ...
                $totalStmt = $this->buildQuery(array());
                $totalStmt->execute();
                $this->totalCount = $totalStmt->fetchColumn();
            }
        }
     ...
     * @param string  $tableName The name of the database table to work with.
     * @param string  $columns Desired columns
     * @return PDODb | array Contains the returned rows from the select query.
     */
     ...
     * @return int
     */
    function loadJsonData($file_path, $table, $transaction = true): array
    {
        ...
        if ($stmt) {
            ...
            if (in_array('SQL_CALC_FOUND_ROWS', $this->queryOptions)) {
                ...
                if ($dbType == 'mysql') {
                    ...
                    $totalStmt = $this->buildQuery(array(), null);
                    $totalStmt->execute();
                    $this->totalCount = $totalStmt->fetchColumn();
                }
            }
